        ...
        function render(data) {
            if (data.blacklisted) {
                summary.innerHTML =
                    '<div class="panel" style="padding:20px; border-left:4px solid #dc2626; margin-bottom:24px;">' +
                    '<div style="font-weight:700; color:#dc2626; margin-bottom:8px;">Your account is currently blacklisted</div>' +
                    '<div style="color:var(--text); font-size:14px;">' + escapeHtml(data.reason || 'Blacklisted by Admin.') + '</div>' +
                    '</div>';
            } else {
                summary.innerHTML = '<div class="empty-panel" style="margin-bottom:24px;">' +
                    "You're in good standing — no active blacklist on your account.</div>";
            }
        }
